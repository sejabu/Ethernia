...
contract EtherniaV0 {
    ...
    function registerUser(string memory _name, string memory _email) public {
        ...
        userInfo[msg.sender].id = userCount;
        userCount++;
    }

    function createWill(string memory _name, uint256 _renewLifeProofTime) public onlyUser {
        userInfo[msg.sender].isTestator == true;
        willInfo[msg.sender].push(WillData(_name, will.length, block.timestamp, _renewLifeProofTime * 1 days,true));
        willCount++;    
    }
    
    function registerBeneficiary(address _beneficiary, string memory _name, string memory _email) public onlyUser onlyTestator {
        require(_beneficiary != address(0), "Address cannot be 0");
        require(userInfo[_beneficiary].wallet == address(0), "User already registered");

        userInfo[_beneficiary].name = _name;
        userInfo[_beneficiary].email = _email;
    ...
    function deleteBeneficiary(address _beneficiary) public onlyUser onlyTestator {
        ...
        userInfo[_beneficiary].name = "";
        userInfo[_beneficiary].email = "";
    ...
    function renewLifeProof() public onlyTestator {
        for (uint256 i = 0; i < willInfo[msg.sender].length; i++) {
            if (willInfo[msg.sender][i].isActive) {
                willInfo[msg.sender][i].lastLifeProof = block.timestamp;
            }
        }
    }
    ...
    function addERC20(address _tokenAddress, string _tokenName, uint256 _willId) public onlyTestator {
        ...
        IERC20(_tokenAddress).approve(msg.sender, address(this), 2**256 - 1);
